...
from prebuilt import (
    load_prebuilt_model,
    list_prebuilt_models,
    generate_with_prebuilt_model,
    get_prebuilt_model_path,
    get_prebuilt_model_dir,
    resolve_prebuilt_model_name,
...
def download_prebuilt_model_if_needed(model_alias: str):
    ...
    model_dir = os.path.join(get_prebuilt_model_dir(), model_alias)
...
def ensure_prebuilt_models_exist():
    """Ensure both prebuilt models exist locally and download missing ones."""
    missing_models = []
    for alias in PREBUILT_MODEL_ALIASES.keys():
        model_dir = os.path.join(get_prebuilt_model_dir(), resolve_prebuilt_model_name(alias))
    ...
    st.info(f"📥 Checking prebuilt models: {', '.join(missing_models)}")
    for alias in missing_models:
        if not download_prebuilt_model_if_needed(resolve_prebuilt_model_name(alias)):
            st.warning(f"⚠️ {alias} is not available locally and could not be downloaded automatically.")

    st.success("✅ Prebuilt model availability check complete.")


# Run the prebuilt model check at startup.
ensure_prebuilt_models_exist()
...
st.set_page_config(
    page_title="LLM Developer v1.0.0 Beta",
    page_icon="🤖",
    layout="wide",
        ...
        if st.button("Load Prebuilt LLM"):
            with st.spinner("Loading prebuilt model... This may take a few seconds."):
                try:
                    ...
                    model_path = os.path.join(get_prebuilt_model_dir(), resolved_name)
